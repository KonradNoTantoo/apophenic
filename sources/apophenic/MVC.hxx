...
public:
	...
	template<class TOther, typename TContext>
	void connect(Hub<TOther> & h, TContext context) { if ( can_connect_to(h) ) h.connect(*this, context); }

	template<class TOther>
	void disconnect() { if( is_connected() ) static_cast<Hub<TOther>*>(_hub)->disconnect(*this); }

	bool is_connected() const { return nullptr != _hub; }
	bool is_connected_to( IHub const & h ) const { return &h == _hub; }
	bool can_connect_to( IHub const & h ) const { return false == is_connected() || is_connected_to( h ); }
...
class Inputer : public Spoke<typename TImplementations::inputer>
{
	...
	using spoke = Spoke<typename TImplementations::inputer>;
	typedef typename TImplementations::inputer TInputer;
