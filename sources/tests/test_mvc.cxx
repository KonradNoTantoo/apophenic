...
struct SynchronousModelFixture : ::testing::Test
{
	...
	void SetUp() override
	{
		EXPECT_CALL(_model, on_registered_cb(An< ap::mvc::Listener<Implementation3,IStringEvent> & >()))
			.Times(3);
...
struct AsynchronousModelFixture : ::testing::Test
{
	...
	void SetUp() override
	{
		EXPECT_CALL(_model, on_registered_cb(An< ap::mvc::Listener<Implementation3,IIntEvent> & >()))
			.Times(3);
		EXPECT_CALL(_model, on_registered_cb(An< ap::mvc::Listener<Implementation3,IStringEvent> & >()))
			.Times(3);
		for ( unsigned i = 0; i < NB_VIEWS; ++i )
